...
async function getStaticProps(context) {
    ...
    const { slug  } = params;
    const fileData = (0,_lib_postUtel__WEBPACK_IMPORTED_MODULE_3__/* .getSinglePost */ .JF)(slug);
    return {
        props: {
            post: fileData
        }
...
async function getStaticPaths() {
    const fileFullNames = (0,_lib_postUtel__WEBPACK_IMPORTED_MODULE_3__/* .getFileNames */ .c6)();
    const fileNames = fileFullNames.map((item)=>item.replace(/\.md$/, ""));
    const fileParams = fileNames.map((name)=>({
            params: {
                slug: name
            }
        }));
...
var __webpack_exports__ = __webpack_require__.X(0, [675,120], () => (__webpack_exec__(754)));
